...
export default function ViewLeadsPage() {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: string, direction: "asc" | "desc" } | null>({ key: "Created Date", direction: "desc" });

  const [debouncedSearch, setDebouncedSearch] = useState("");
  const [totalLeads, setTotalLeads] = useState(0);
  const [leadMetricsData, setLeadMetricsData] = useState({ total: 0, active: 0, accepted: 0, cancelled: 0 });
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (sortConfig) {
          let sortBy = "lead_created_at";
          if (sortConfig.key === "Lead ID") sortBy = "lead_reference";
          else if (sortConfig.key === "Company Name") sortBy = "employer.employer_name";
          else if (sortConfig.key === "Contact Person") sortBy = "contact.contact_first_name";
          else if (sortConfig.key === "Employees") sortBy = "employer.number_of_employees";
          else if (sortConfig.key === "Status") sortBy = "lead_status";
          else if (sortConfig.key === "Created Date") sortBy = "lead_created_at";
        ...
        if (data && (data as any).pagination) {
          setTotalLeads((data as any).pagination.total || 0);
        } else {
          setTotalLeads((data || []).length);
        }

        if (data && (data as any).metrics) {
          setLeadMetricsData((data as any).metrics);
        }
      } catch (error) {
  ...
  const leadMetrics = [
    { label: "Total Leads", value: leadMetricsData.total },
    { label: "Active", value: leadMetricsData.active },
    { label: "Accepted", value: leadMetricsData.accepted },
    { label: "Cancelled", value: leadMetricsData.cancelled },
  ]
            ...
            <Box sx={{
              ...
              <Box sx={{
                ...
                <Typography sx={{
                  ...
                  Showing {(safePage - 1) * PAGE_SIZE + 1} to {Math.min(safePage * PAGE_SIZE, totalLeads)} of {totalLeads} entries
                </Typography>
