...
export function normaliseQuote(raw: any): Quote {
  return {
    ...
    companyName: raw.lead?.employer?.employer_name ?? raw.employer?.employer_name ?? raw.companyName ?? "",
    ...
    province: raw.lead?.employer?.province ?? raw.province,
    industry: raw.lead?.employer?.industry_type ?? raw.industry,
    monthlyPremium: raw.total_premium ?? raw.monthly_premium ?? raw.monthlyPremium ?? 0,
    coverageAmount: raw.coverage_amount ?? raw.coverageAmount ?? (raw.benefits ? raw.benefits.reduce((sum: number, b: any) => sum + (parseFloat(b.cover_amount) || 0), 0) : 0),
