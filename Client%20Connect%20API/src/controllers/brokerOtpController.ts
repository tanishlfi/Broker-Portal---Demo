  ...
  try {
    ...
    if (referenceType === "Quote") {
      const quote = await BrokerQuote.findOne({
        where: {
          [Op.or]: [{ quote_id: referenceId }, { quote_reference: referenceId }]
        },
        include: [{ 
      ...
      const lead = await BrokerLead.findOne({
        where: {
          [Op.or]: [{ lead_id: referenceId }, { lead_reference: referenceId }]
        },
        include: [{ model: BrokerContact, as: "contact" }]
      });
...
export const verifyOTP = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (otpRecord.reference_type === "Quote") {
      const quote = await BrokerQuote.findOne({
        where: { [Op.or]: [{ quote_id: referenceId }, { quote_reference: referenceId }] },
        transaction: t
      });
      if (quote) {
        await quote.update({
      ...
      await BrokerLead.update(
        { lead_status: "Accepted" },
        { where: { [Op.or]: [{ lead_id: referenceId }, { lead_reference: referenceId }] }, transaction: t }
      );
      // Let's resolve the actual lead_id UUID for the onboarding service trigger
      const leadObj = await BrokerLead.findOne({
        where: { [Op.or]: [{ lead_id: referenceId }, { lead_reference: referenceId }] },
        attributes: ["lead_id"],
        transaction: t
      });
      triggerOnboarding = true;
      leadIdToOnboard = leadObj ? leadObj.lead_id : referenceId;
    }
